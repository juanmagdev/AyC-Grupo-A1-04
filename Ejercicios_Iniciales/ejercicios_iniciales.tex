...
\begin{questions}
    ...
    \begin{parts}
        ...
        \part Para un programa $P$, con entrada $X1=n$ que calcula la función $H(n)$ con complejidad temporal
$T(n)$, ¿Cuál sería el máximo resultado que podría devolver?
        \begin{solution}
            \\
            Debido a las restricciones impuestas de tiempo, podemos tener N número de pasos de cómputo.
            Contamos con asignaciones y bucles. Los bucles no nos sirven pues son siempre de la forma while XN != 0 do o sea  que tenemos que aumentar una variable X2, por ejemplo, para que dentro del bucle esta disminuya y X1 aumente lo cual es absurdo porque estás duplicando un trabajo y perdiendo instrucciones por el camino. La solución pues consiste en usar N veces la asignación X1 = X1 +1 lo cual nos quedaría N + sumatorio de uno a n del número 1 lo cual es N+N = 2N
            \\
            \\
            Esto se ve mucho mejor si usamos máquinas de Turing. Supongamos que en nuestro alfabeto solo poseemos el símbolo "|" (al que llamaremos palo) y el símbolo "\#" (al que llamaremos cadena vacía), en la cinta ya poseemos N palos y la cabeza lectora se encuentra a la derecha de la entrada (porque sino los N pasos nos lo gastamos recorriendo la entrada). Con todo esto, para aumentar el número de palos lo único que podemos hacer es ir hacia la derecha escribiendo palos. Y al dar N pasos y, ya de entrada tener N palos. N+N = 2N
